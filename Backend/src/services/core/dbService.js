// Recursively replace all undefined values with null in an object or array
...
/**
 * Saves the full result of a pipeline process to Firestore.
 * Uses the source-provided ID as the document ID to prevent duplicates.
 * @param {object} pipelineData The complete data object from the pipeline service.
 * @returns {Promise<string>} The document ID.
 */
async function savePipelineResult(pipelineData) {
  try {
    // Use the source-specific ID as the document ID to prevent duplicates
    const cleanedData = replaceUndefinedWithNull(pipelineData);
    const docRef = db.collection('pipeline_results').doc(cleanedData.id);
    await docRef.set(cleanedData);
    console.log(`Successfully saved full pipeline result for ID: ${cleanedData.id}`);
    return docRef.id;
  } catch (error) {
    console.error(`Error saving pipeline result for ID ${pipelineData.id} to Firestore:`, error);
    throw error;
  }
}

/**
 * Saves the data from a failed pipeline process to a separate collection for debugging.
 * @param {object} pipelineData The data object from the pipeline, including the error.
 * @returns {Promise<string>} The document ID.
 */
async function saveErrorResult(pipelineData) {
  try {
    const cleanedData = replaceUndefinedWithNull(pipelineData);
    const docRef = db.collection('pipeline_errors').doc(cleanedData.id);
    await docRef.set(cleanedData);
    console.log(`Successfully saved error pipeline result for ID: ${cleanedData.id}`);
    return docRef.id;
  } catch (error) {
    console.error(`Error saving error result for ID ${pipelineData.id} to Firestore:`, error);
    throw error;
  }
}
